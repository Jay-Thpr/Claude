...
async function runAnthropicPrompt(prompt: string): Promise<string> {
...
type TaskStartDeps = {
  runAnthropicPrompt?: (prompt: string) => Promise<string>;
...
export async function handleTaskStartRequest(request: Request, deps: TaskStartDeps = {}): Promise<Response> {
  ...
  const anthropic =
    deps.runAnthropicPrompt ?? deps.runGeminiPrompt ?? runAnthropicPrompt;
