export interface ScamSignals {
  ...
  hasSensitiveDataRequest: boolean;
...
function detectBrandMismatch(
  domain: string,
  text: string
): { mismatch: boolean; brand: string | null } {
...
export function extractScamSignals(url: string, text = ""): ScamSignals {
...
export function signalsToPromptContext(signals: ScamSignals): string {
  const lines: string[] = ["Pre-analysis signals:"];
  lines.push(`- HTTPS: ${signals.isHttps ? "yes" : "NO (insecure connection)"}`);
  if (signals.hasSuspiciousTld) lines.push(`- Suspicious TLD: .${signals.tld}`);
  ...
  if (signals.signalCount === 0) lines.push("- No obvious heuristic signals detected");
  return lines.join("\n");
}
